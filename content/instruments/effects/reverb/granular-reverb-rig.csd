...
groupbox bounds(10, 160, 320, 130) text("GRAIN PARAMETERS") colour(40, 40, 60) fontColour(200, 200, 220) {
    rslider bounds(10, 25, 50, 50) channel("density") range(5, 200, 40, 0.4, 1) text("Density") textColour(200,200,220) trackerColour(140, 200, 120)
    rslider bounds(65, 25, 50, 50) channel("grain_dur") range(0.01, 0.5, 0.08, 0.3, 0.001) text("Size") textColour(200,200,220) trackerColour(140, 200, 120)
    rslider bounds(120, 25, 50, 50) channel("amp_jitter") range(0, 0.5, 0.15, 1, 0.01) text("Amp Jit") textColour(200,200,220) trackerColour(140, 200, 120)
    rslider bounds(175, 25, 50, 50) channel("pitch_jitter") range(0, 0.1, 0.01, 0.5, 0.001) text("Pitch Jit") textColour(200,200,220) trackerColour(140, 200, 120)
    rslider bounds(230, 25, 50, 50) channel("grain_bright") range(500, 16000, 8000, 0.4, 1) text("Bright") textColour(200,200,220) trackerColour(140, 200, 120)
...
groupbox bounds(335, 160, 155, 130) text("FEEDBACK") colour(40, 40, 60) fontColour(200, 200, 220) {
    rslider bounds(10, 25, 55, 55) channel("feedback") range(0, 0.9, 0.5, 1, 0.01) text("Amount") textColour(200,200,220) trackerColour(200, 180, 100)
...
instr 99
  ...
  agrain_L grain 0.5, 1, kdensity, kampjit, kpitchjit, kgdur, gi_buf, gi_win, kgdur + 0.01, 0

  ; --- Right grain stream (slightly different density for decorrelation) ---
  kdensity_R = kdensity * (1 + kspread * 0.15)
  agrain_R grain 0.5, 1, kdensity_R, kampjit, kpitchjit, kgdur, gi_buf, gi_win, kgdur + 0.01, 0
